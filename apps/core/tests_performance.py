...
class PerformanceTestBase(TransactionTestCase):
    ...
    def measure_queries(self, func, *args, **kwargs):
        """
        ИСПРАВЛЕНО: Правильное измерение количества запросов
        Использует CaptureQueriesContext вместо reset_queries()
        """
...
class QueryCountTests(PerformanceTestBase):
    ...
    def test_object_detail_query_count(self):
        ...
        # ИЗ ЛОГОВ: в последнем запуске 32 запроса
    ...
    def test_participant_detail_query_count(self):
        ...
        # ИЗ ЛОГОВ: в последнем запуске 63 запроса
...
class PerformanceBenchmarkTests(PerformanceTestBase):
    ...
    def test_object_list_response_time(self):
        ...
        # Проверяем количество запросов (должно быть 6)
    ...
    def test_object_detail_response_time(self):
        ...
        # Проверяем количество запросов (должно быть около 32)
        self.assertLess(metrics['query_count'], 40)
        self.assertGreater(metrics['query_count'], 20)
    ...
    def test_graph_response_time(self):
        ...
        # Проверяем количество запросов (должно быть 3)
