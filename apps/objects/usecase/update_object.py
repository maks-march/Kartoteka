...
class UpdateObjectUseCase:
    ...
    def execute(self, obj_id: int, name: str, level: int, category_id: int) -> ObjectEntity:
        self._validate_input(name, level, category_id)

        # Проверяем, что объект существует
        existing = self.object_repo.get_by_id(obj_id)
        if existing is None:
        ...
        entity = ObjectEntity(id=obj_id, name=name, level=level, category_id=category_id)
        return self.object_repo.update(entity)
